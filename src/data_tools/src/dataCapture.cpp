...
public:
    ...
    std::thread* keyboardInterruptCheckingThread;   // 键盘中断检查线程
    std::thread* monitoringThread;  // 监控线程
    bool keyboardInterrupt = false; // 键盘中断标志
    bool captureStop = false;   // 停止捕获标志
    std::mutex captureStopMtx;  // 停止捕获锁
    ...
    DataCapture(std::string datasetDir, int episodeIndex, int hz, int timeout, double cropTime=-1, bool useService=false): DataUtility(datasetDir, episodeIndex) {
        ...
        // 初始化订阅者
        for(int i = 0; i < cameraColorNames.size(); i++){   // 遍历所有相机颜色传感器
            unused = system((std::string("mkdir -p ") + cameraColorDirs.at(i)).c_str());    // 创建目录 unused 用于忽略返回值防止编译器警告
            
            subCameraColors.push_back(nh.subscribe<sensor_msgs::Image>(
                cameraColorTopics[i],   // 话题名称
                2000,   //消息队列长度
                boost::bind(&DataCapture::cameraColorHandler, this, _1, i),   // 绑定回调函数和索引
                ros::VoidConstPtr(), ros::TransportHints().tcpNoDelay()
            ));

            subCameraColorConfigs.push_back(nh.subscribe<sensor_msgs::CameraInfo>(
                cameraColorConfigTopics[i], 
                2000, 
                boost::bind(&DataCapture::cameraColorConfigHandler, this, _1, i), 
                ros::VoidConstPtr(), ros::TransportHints().tcpNoDelay()
            ));
        }
    ...
    void join(){
        // 等待所有线程结束并释放相关资源
        for(int i = 0; i < cameraColorNames.size(); i++){
            cameraColorSavingThreads.at(i)->join(); // 等待线程结束
            delete cameraColorSavingThreads.at(i);  // 释放线程资源
            cameraColorSavingThreads.at(i) = nullptr;   // 避免野指针
        }
        for(int i = 0; i < cameraDepthNames.size(); i++){
        ...
        monitoringThread->join();   // 等待监控线程结束
        delete monitoringThread;
        monitoringThread = nullptr;
        data_msgs::CaptureStatus captureStatus; // 发布捕获状态
        captureStatus.quit = true;
        pubCaptureStatus.publish(captureStatus);
    }
    // ROS 订阅回调函数 处理订阅的传感器数据
    // 这里通过color相机的帧长，200帧时，停止数据采集
    void cameraColorHandler(const sensor_msgs::Image::ConstPtr& msg, const int& index){
        cameraColorMsgDeques.at(index).push_back(*msg); // 将消息添加到缓存队列
        std::lock_guard<std::mutex> lock(cameraColorMsgCountMtxs.at(index));    // 线程安全锁
        if(cameraColorMsgCounts.at(index) == 0){
            cameraColorStartTimeStamps.at(index) = msg->header.stamp.toSec();   // 如果是第一帧，记录起始时间戳
        }else{
            cameraColorLastTimeStamps.at(index) = msg->header.stamp.toSec();    // 否则更新最后时间戳
        }
        cameraColorMsgCounts.at(index) += 1;    // 增加消息计数
        cameraColorFrameIds.at(index) = msg->header.frame_id;   // 更新帧ID
        if(cameraColorMsgCounts.at(index) >= 200){ // 达到指定帧数，停止捕获，好像得+1
            shutdown();
        }   
    }
    ...
    void printProgressBar(int current, int total) {
        ...
        const int barLength = 50; // 进度条长度（字符数）
        float progress = static_cast<float>(current) / total;
        int filledLength = static_cast<int>(barLength * progress);
        ...
        // 输出进度条（\r回到行首覆盖，\033[K清除剩余内容避免乱码）
        std::cout << "\rCamera color progress: [" << bar << "] "
    ...
    void monitoring(){
        ...
        const int targetFrame = 200;  // 目标采集帧数
        while(true){
            ...
            // 打印总帧数后，添加进度条
            std::cout << "sum total frame: " << allCount << std::endl;
            // 获取cameraColorMsgCounts[0]的当前值（线程安全）
            int currentCount = 0;
            if (!cameraColorNames.empty()) { // 避免空容器访问
                cameraColorMsgCountMtxs.at(0).lock();
                currentCount = cameraColorMsgCounts.at(0);
                cameraColorMsgCountMtxs.at(0).unlock();
                
                // 打印进度条
                printProgressBar(currentCount, targetFrame);
            }
